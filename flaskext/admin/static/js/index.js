...
$(document).ready(function(){
    $.each($('textarea'), function(i, val){
        ...
        try {
            ...
            $(val).val(JSON.stringify(j));
            $(val).addClass('jsonable');
            // если не вылетело исключение, значит поле с json данными
            // добавляю к полю блок для редактора
            var id_ = $(val).attr('id') + '_edit';
            var id = $(val).attr('id');
            $('<div class="editor">').attr('id', id_).insertAfter(val);
            $('<div class="buttons">' +
                    '<a class="butt" href="javascript:load_from_box(\''+id+'\',\''+id_+'\');">&darr;</a>&nbsp;&nbsp;' +
                    '<a class="butt"  href="javascript:extract_json(\''+id+'\');">&uarr;</a>' +
                    '</div>').insertAfter(val);
            // добавляю функционал редактора к созданному на лету блоку
            json_editor(id_, $(val).val());
        } catch (e){
            // console.log(e);
        }
...
function apply_editlets() {
    ...
    // add the "new" buttons
    var add_more_box = $('<div class="inline_add_box"><div class="add_box_content">добавить: <a data-task="add_value" href="#">текст</a> | <a data-task="add_array" href="#">список</a> | <a data-task="add_object" href="#">словарь</a></div></div>');
...
function load_from_box(from, to) {
    $(to).html('');
    json_editor($(to).attr('id'), $('#'+from).val());
